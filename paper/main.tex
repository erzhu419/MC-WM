...
(3 seeds, mean shown; $50$k real-env steps except ``offline-only''
which uses D4RL medium-replay + $1024$-step sim warm-up).
Offline-only is a clear lower bound; full H2O+ ($r=1.0$) improves
dramatically with more offline data; pure online SAC falls
well short of both H2O+ and our MC-WM.}
...
\midrule
H2O+ offline-only (lower bound)                  & $-366$        \\
Pure online SAC (no sim, no offline)             & $1460$        \\
H2O+ offline fraction $r=0.1$                    & \textsc{tbd}$^*$ \\
H2O+ offline fraction $r=0.25$                   & \textsc{tbd}$^*$ \\
H2O+ offline fraction $r=0.5$                    & \textsc{tbd}$^*$ \\
H2O+ offline fraction $r=1.0$ (\textit{full})    & $2099$        \\
\textbf{MC-WM c9 (ours)}                         & $\mathbf{6288}$ \\
c4 oracle (MBPO-on-real)                         & $6032$        \\
\bottomrule
\end{tabular}
\par\smallskip
\footnotesize
$^*$\,Running; numbers in camera-ready.
\end{table}

The decomposition supports three claims.  (i) \emph{Offline-only H2O+ is
a lower bound.}  With no sim rollouts it drops to $-366$, an order of
magnitude below every other configuration.  The offline AWR update
cannot recover from the sim-real gap on its own.  (ii) \emph{Pure
online SAC is a middle ground but far from competitive.}  Even with
$50$k clean real-env steps and no sim-to-real artefacts, model-free
SAC attains only $1460$---less than half of what the sim+offline
combination in H2O+ buys, and roughly $23\%$ of MC-WM's return.
(iii) \emph{H2O+ depends strongly on offline-data quantity} (numbers
in running experiments); this is a structural limitation for deployments
where a large offline corpus is unavailable, which is precisely the
regime MC-WM is designed for---our method needs only the $50$k real
transitions already required for residual fitting.
