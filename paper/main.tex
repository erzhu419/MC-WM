...
``c9 (legacy)'' is MC-WM without the RAHD stabilization module of
\S\ref{sec:rahd}; ``c9 RAHD'' adds RAHD/P0/P1 and is reported only on the
environment for which we ran a full RAHD ablation
(\texttt{gravity\_soft\_ceiling}, see Table~\ref{tab:h2o_decomp}).
Per-seed numbers for both configurations are in
...
gravity\_soft\_ceiling     & 494    & 8.9   & 6032 & 0.52  & 2099  & n/a  & 6288 & 2.0  & \textbf{4919} & \textbf{0.82} \\
...
(c1 / c4 / H2O+) are unchanged.  We separate two MC-WM configurations:
\textbf{c9 (legacy)} omits the RAHD module and matches the
configuration of all earlier ablation tables; \textbf{c9 RAHD} enables
P0 warmup, P1 belief-conditioned $\QD$, and the RAHD-A/B/C hooks of
\S\ref{sec:rahd}.  RAHD trades $\sim 22\%$ of return for $\sim 88\%$
fewer violations, $\sim 4{\times}$ lower seed variance, and the
data-budget-matched H2O+ comparison of \S\ref{sec:h2o_analysis}.  We
report c9~RAHD only on \texttt{gravity\_soft\_ceiling} since the
controlled three-seed RAHD sweep was performed only there; running
RAHD on the other two environments is part of our concrete asks for
the rebuttal (Section~\ref{sec:experimental-todo}).

\paragraph{Per-environment summary (c9 legacy).}
First, on \texttt{gravity\_ceiling} our method yields a $9.6\times$
return over the raw-sim transfer baseline ($5196$ vs.\ $543$) while
keeping violations below $0.05$/episode, and reaches $89\%$ of oracle
performance ($5196$ vs.\ c4's $5868$).  Second, on
\texttt{gravity\_soft\_ceiling} our method matches the oracle ($6288$
vs.\ $6032$) and out-performs H2O+ by $3\times$ at $r=1.0$.  Third,
on \texttt{friction\_walker\_soft\_ceiling} our method beats \emph{even}
the oracle ($357$ vs.\ $254$, a $1.4\times$ improvement) because the
oracle's policy exploits the learned model into the constraint-violating
region whereas our constraint filter suppresses those rollouts; the
small absolute gain reflects this benchmark's small dynamics gap
($\approx 0.34$ residual improvement in Table~\ref{tab:envs}).
...
\midrule
\textbf{MC-WM RAHD c9 (ours)}                    & $50$k pretrain & $50$k & 0                  & $\mathbf{4919}$ & $\mathbf{187}$ & $\mathbf{9}$ \\
...
Table~\ref{tab:p1-isolation} shows the controlled comparison: identical
seeds $\{42,123,456\}$, identical RAHD-A/B/C, identical P0 warmup,
identical LLM oracle (GLM-5.1), identical $\gamma_\Delta=0.5$; only the
\texttt{--qdelta\_belief\_sig} flag is toggled (P1 on/off).  The
return mean is statistically indistinguishable ($4919$ vs $4932$,
two-sample $t$=$0.04$, $p=0.97$) but the seed-spread collapses from
$23.8\%$ to $9.2\%$ ($F$-test $p=0.046$).  This isolates the
$22\%\!\to\!9\%$ variance reduction to P1 specifically.
