...
bounding the on-policy expected-value gap
$|J(\pi;\Mreal)-J(\pi;\Mresid)|$ by the visitation-weighted
total-variation distance on $\pi$'s support, scaled by
$\tfrac{2\gamma\Rmax}{(1-\gamma)^2}$ (\emph{not} a sup-norm bound,
which would require all-state TV control; see
Remark~\ref{rmk:no-supnorm}); (ii) a
...
$9.6\times$ and $11.4\times$ the return of naive sim-transfer
(\texttt{gravity\_ceiling}: $5220\!/\!543$;
 \texttt{gravity\_soft\_ceiling}: $5653\!/\!494$, post-fix RAHD c9).
On \texttt{friction\_walker\_soft\_ceiling}, where the dynamics gap is
small ($\approx\!0.34$ residual improvement), MC-WM \emph{under-performs}
raw sim ($244$ vs $262$, $-7\%$); we report this rather than the
inflated legacy $357$, which was an artefact of a buffer-weight bug
(\S\ref{sec:bugfixes}).  On \texttt{gravity\_soft\_ceiling} the method
matches an oracle that trains on real data directly and beats H2O+
by $2.6\times$ at $r=1.0$.  An ablation shows each of
...
\item Empirically, $9.6\times$ and $11.4\times$ returns over naive
sim transfer on the two large-gap physics benchmarks
(post-fix RAHD c9 on \texttt{gravity\_ceiling} / \texttt{gravity\_soft\_ceiling}),
plus a clean negative-result entry on the smallest-gap benchmark
(\texttt{friction\_walker\_soft\_ceiling}: $0.93\times$ raw sim,
correctly classified by our diagnostic threshold);
a component ablation with the corrected
\texttt{--disable\_qdelta\_filter} flag; and a single-morphology
gap-taxonomy sweep ($11$ envs) that bootstraps a $0.32$ residual-
improvement threshold for when the method should be expected to help.
\end{itemize}
...
on the right-hand side.  All downstream uses of
Theorem~\ref{thm:simulation} in this paper (corollaries, intro, abstract)
state the bound in expected-value form; if the reader sees a sup-norm
phrasing it is a bug we should be told about.
\end{remark}
...
\texttt{friction\_walker\_soft\_c.} & friction: sim $0.3\times$ vs.\ real $1\times$ & soft forward-velocity cap & $\approx 0.34$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{friction\_walker\_soft\_ceiling}: Walker2D with sim
ground friction at $0.3\times$ the real value, and a soft
forward-velocity cap penalty.  (Earlier drafts of this text mis-stated
the direction as ``friction $1\!\times\!\to\!4\!\times$''.)
\end{itemize}
...
$\sim\!100$k real-env transitions (Phase~1b collects $50$k for
residual fitting; Phase~2 then runs another $50$k of on-policy
real-env steps for SAC training).  An earlier draft mis-stated this
as $50$k; the corrected number is what \texttt{step2\_mbrl\_residual.py}
actually consumes (see lines 842, 1050, 1200 of the script).  The
data-matched H2O+ comparison in §\ref{sec:h2o_analysis} is therefore
not strictly equal-budget---H2O+ at $r=0.25$ uses $\sim\!50$k offline
plus $50$k sim (no real-env), whereas MC-WM uses
$\sim\!100$k real plus $200$k sim.  The $r=1.0$ H2O+ row uses
$\sim\!250$k total transitions and is closer to budget-matched.  We
re-report the comparison with this honest accounting in
Table~\ref{tab:h2o_decomp}.
\end{itemize}
...
gravity\_soft\_ceiling     & 494    & 8.9   & 6032 & 0.52  & 2099  & n/a  & 6288 & 2.0  & \textbf{5653$\!\pm\!$696} & \textbf{7.03}  \\
...
\midrule
\textbf{MC-WM RAHD c9 (ours, true post-fix)}     & $200$k pretrain & $\sim\!100$k & 0                  & $\mathbf{5653}$ & $\mathbf{696}$ & $\mathbf{12}$ \\
...
\midrule
\textbf{Full c9 RAHD (true post-fix)}                 & $\mathbf{5653}$ & $\mathbf{696}$ & $7.03$ \\
\quad -- $\QD$ filter (\texttt{--disable\_qdelta\_filter}; true ablation) & $5050$          & $403$          & $1.07$ \\
\midrule
\textit{Legacy c9 (no-RAHD pipeline; for context only):} \\
\quad Legacy ``full c9''                              & $5394$          & $478$          & $4.68$ \\
\quad -- Pareto gate                                  & $5299$          & $677$          & $1.03$ \\
\quad -- Bellman $\QD$ ($\gamma_\Delta=0$, NOT a filter ablation) & $5410$ & $113$       & $3.09$ \\
\quad -- Role \#5 (fixed HP)                          & $5351$          & $223$          & $13.52$ \\
\quad -- LLM (Roles off)                              & $5274$          & $311$          & $1.65$ \\
\bottomrule
\end{tabular}
\par\smallskip
\footnotesize
The top block is the only \emph{true} RAHD ablation (3 seeds, post-fix
pipeline matching Table~\ref{tab:main}).  The bottom block, kept for
context, is the legacy c9 pipeline (no RAHD/P0/P1) and was previously
mis-labelled ``full c9 RAHD'' in earlier drafts; we now label it
explicitly to match \texttt{run\_E4\_ablation\_matrix.sh} which states
``no RAHD module''.  The legacy ``-- Bellman $\QD$ ($\gamma_\Delta=0$)''
line is also \emph{not} a true Q$_\Delta$-filter ablation: setting
$\gamma_\Delta=0$ falls back to the per-step weight $1/(1+\mathrm{MSE}/\tau)$,
which still filters rollouts.  The new
\texttt{--disable\_qdelta\_filter} flag is the genuine ablation
($w_{\mathrm{qd}}\!\equiv\!1$).
\end{table}

\paragraph{Three findings the true multi-seed ablation forces us to
revise once more.}

\emph{(F1, revised) $\QD$ contributes to both reward and safety.}  The
\emph{true} Q$_\Delta$-filter ablation
(\texttt{--disable\_qdelta\_filter}) drops return $5653\!\to\!5050$
(a $11\%$ regression) and lowers mean violations $7.03\!\to\!1.07$.
The Q$_\Delta$ filter therefore contributes \emph{positively} to
return (modulo seed variance discussed below) and \emph{negatively}
to safety in the typical case.  The earlier interpretation in our
draft that ``$\QD$ is a safety filter, not a reward driver'' was an
artefact of (i) the legacy $\gamma_\Delta\!=\!0$ ablation still
applying a per-step trust weight and (ii) the legacy E4 ablation
matrix not actually enabling RAHD.

A further nuance: in 2 of 3 seeds RAHD strictly dominates the no-Q$_\Delta$
configuration on \emph{both} reward and safety
(\textbf{s$_{42}$}: $5974$ at $0.10$ vs $5332$ at $0.13$;
\textbf{s$_{123}$}: $6297$ at $0.13$ vs $5338$ at $2.70$).  In the
remaining seed (\textbf{s$_{456}$}) RAHD is only marginally better on
return ($4687$ vs $4481$) but shifts violations up dramatically
($20.87$ vs $0.37$): the Q$_\Delta$ filter combined with the
belief-conditioned critic apparently lets a high-reward
high-violation mode through in this seed.  We report mean$\pm$std
honestly above; a per-seed table appears in
...
\emph{(T3) The $35\%$-residual diagnostic survives the sweep, but
needs a gap-type qualifier.}  We re-fit the threshold by bootstrapping
a logistic regression of ``works'' against the residual-improvement
ratio over $16$ envs (the $11$ E5 sweep envs $+$ the original $5$
cross-morphology envs).  The point estimate is $\mathbf{0.316}$
(31.6\%) with a $\mathbf{95\%}$ bootstrap CI of $[-0.067,\,0.485]$ and
inter-quartile range $[0.208,\,0.397]$ (Fig.~\ref{fig:threshold-fit}).
The prior heuristic $0.35$ lies within the IQR.  The wide CI is
attributable to the observation-noise sweep: \texttt{obs\_noise\_05}
has gap $0.61$ but works, while \texttt{obs\_noise\_10/20/50} have
gap $0.27\!-\!0.46$ and fail---i.e.\ residual-improvement ratio alone
is \emph{not} sufficient to separate works from fails when the gap
is observation-level.  We therefore replace the original
``$\mathrm{gap}\ge 35\%\Rightarrow$ works'' diagnostic with a
two-stage rule: (1) check \emph{type} of gap (physics-dynamics vs.\
observation/actuator); (2) within physics-dynamics gaps, the $35\%$
threshold holds.

\begin{figure}[h]
\centering
\includegraphics[width=0.78\linewidth]{figures/gap_taxonomy_threshold_fit.pdf}
\caption{\textbf{Re-fit of the gap-taxonomy diagnostic threshold} on
the $11$ E5 single-factor sweep envs plus the $5$ cross-morphology
envs (16 total, 3 seeds each).  Each point is one env at its
mean residual-improvement ratio (x) and post-fix RAHD c$9$ return
(y); marker style indicates the binary ``works'' label
(\texttt{works} $\Leftrightarrow$ return $> 1.5\!\times$ raw sim).
Logistic-regression fit by bootstrap ($n=2000$) gives a point
estimate of $0.316$ (dashed line) with IQR $[0.21, 0.40]$ (gray
band).  The prior $0.35$ heuristic (dotted line) lies in the IQR.
The mismatch between low-gap obs-noise envs that fail and high-gap
obs-noise envs that succeed is what widens the CI; see (T3) for the
two-stage diagnostic.}
\label{fig:threshold-fit}
\end{figure}
...
\emph{(delivered)}.}  Table~\ref{tab:main} now reports MC-WM RAHD c$9$
on all three environments at $3$ seeds each: \texttt{gravity\_ceiling}
$5220\!\pm\!181$, \texttt{gravity\_soft\_ceiling} $5653\!\pm\!696$,
\texttt{friction\_walker\_soft\_ceiling} $244\!\pm\!15$.  Per-seed
numbers are in Appendix~\ref{app:seeds}.  The
\texttt{friction\_walker\_soft\_ceiling} number is below raw sim and
was correctly flagged by our gap-taxonomy diagnostic
(§\ref{sec:taxonomy}) as a fail case.
...
\paragraph{Caveat (sup-norm vs.\ expected-value).}
The argument above is a pointwise sup-norm bound under the
restriction of $\eps$ to $\supp(\dpi_{\Mreal})$.
Remark~\ref{rmk:no-supnorm} in the main text shows this restriction is
\emph{not} valid in general: a pair of MDPs that agree on the support
but differ on unreachable states has $\eps=0$ yet a non-zero pointwise
$\|e\|_\infty$.  The headline statement of
Theorem~\ref{thm:simulation} (Section~\ref{sec:theory}) is therefore
the expected-value form
$|J(\pi;\Mreal)-J(\pi;\Mresid)|\le\tfrac{2\gamma\Rmax}{(1-\gamma)^2}
\mathbb E_{(s,a)\sim d^\pi_{\Mreal}}[\mathrm{TV}(\Preal,\Presid)]$,
which is what our Lean development discharges; the appendix derivation
above is retained as an illustration of the operator-level argument
under the strictly stronger sup-norm $\eps_\infty:=
\mathrm{TV}_{\mathcal S\times\mathcal A}(\Preal,\Presid)$
(replace $\eps$ by $\eps_\infty$ in Step~3 to recover the
sup-norm-correct version).
