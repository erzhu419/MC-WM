"""
快速 MVP：WindHopper（obs=11, act=3）
50k steps, RE-SAC (3 critics)

v2: simple seed + smooth gate
v3: MixedReplayBuffer 50/50 + smooth gate
v4: MixedReplayBuffer confidence-weighted + smooth gate

运行：conda run -n MC-WM python3 -u experiments/mvp_fast.py --mode v2
"""
...
def make_logger(mode):
    log_file = f"/tmp/mvp_fast_{mode}.log"
    def log(msg):
...
class ReplayBuffer:
    ...
    def add(self, s, a, r, s2, d, confidence=1.0):
        i = self.ptr
...
class MixedReplayBuffer:
    """Dual-partition: frozen real + circular sim. Fixed real_ratio per batch."""

    def __init__(self, obs_dim, act_dim, real_data, sim_capacity,
                 real_ratio=0.5, confidence_weighted=False):
        self.real_ratio = real_ratio
        self.confidence_weighted = confidence_weighted
        # Frozen real partition
        ...
        self.n_real = n_real
        # Circular sim partition
        self.sim_cap = sim_capacity
        self.sim_ptr = self.sim_size = 0
        ...
        self.sim_conf = np.ones(sim_capacity, np.float32)  # confidence scores

    @property
    def size(self):
        return self.n_real + self.sim_size

    def add(self, s, a, r, s2, d, confidence=1.0):
        i = self.sim_ptr
        self.sim_s[i]=s; self.sim_a[i]=a; self.sim_r[i]=r
        self.sim_s2[i]=s2; self.sim_d[i]=d
        self.sim_conf[i] = confidence
        self.sim_ptr = (i+1) % self.sim_cap
        self.sim_size = min(self.sim_size+1, self.sim_cap)

    def sample(self, n):
        n_r = int(n * self.real_ratio)
        n_s = n - n_r
        r_idx = np.random.randint(0, self.n_real, n_r)
        if self.sim_size > 0:
            if self.confidence_weighted:
                # Weighted sampling: high-confidence sim transitions sampled more
                w = self.sim_conf[:self.sim_size].copy()
                w = w + 0.01  # floor to avoid zero-prob
                w = w / w.sum()
                s_idx = np.random.choice(self.sim_size, n_s, replace=True, p=w)
            else:
...
class GatedCorrectedEnv:
    def __init__(self, env_cls, corrector, tau, eps_jac):
        ...
        self._last_obs = None
        self._g_smooth = None
        self.last_gate_score = 1.0  # exposed for buffer confidence
    ...
    def reset(self, **kw):
        ...
        self._g_smooth = None  # reset smoothing on episode boundary
        self.last_gate_score = 1.0
        return obs, info
    ...
    def gate_stats(self):
        ...
        return {"mean": float(h.mean()), "median": float(np.median(h)),
                "p10": float(np.percentile(h, 10)), "p90": float(np.percentile(h, 90)),
...
# Collect + SINDy
# ─────────────────────────────────────────────
def collect_paired(env_cls, n_steps, log_fn, seed=SEED):
    sim = env_cls(mode="sim"); real = env_cls(mode="real")
    SA_list, ds_list, real_transitions = [], [], []
    ...
    for _ in range(n_steps):
        ...
        real_transitions.append((or_.copy(), a.copy(), rr, nr.copy(), float(dr and not tr)))
        os_, or_ = ns, nr
    ...
    log_fn(f"  Collected {len(SA)} paired steps ({ep} eps)")
    return SA, delta_s, real_transitions
...
# Train
# ─────────────────────────────────────────────
def train(env_fn, env_cls, label, log_fn, mode, seed=SEED, real_data=None):
    ...
    if mode == "v2":
        # Simple seed into circular buffer
        buf = ReplayBuffer(od, ad, REPLAY_SIZE)
        if real_data:
            for s, a, r, s2, d in real_data:
                buf.add(s, a, r, s2, d)
            log_fn(f"  Seeded buffer with {len(real_data)} real transitions (will be diluted)")
    elif mode == "v3":
        # RLPD-style fixed 50/50
        buf = MixedReplayBuffer(od, ad, real_data, REPLAY_SIZE,
                                real_ratio=REAL_RATIO, confidence_weighted=False)
        log_fn(f"  MixedBuffer: {len(real_data)} real (frozen {REAL_RATIO*100:.0f}%) uniform sim")
    elif mode == "v4":
        # Confidence-weighted sim sampling
        buf = MixedReplayBuffer(od, ad, real_data, REPLAY_SIZE,
                                real_ratio=REAL_RATIO, confidence_weighted=True)
        log_fn(f"  MixedBuffer: {len(real_data)} real (frozen {REAL_RATIO*100:.0f}%) confidence-weighted sim")
    else:
        buf = ReplayBuffer(od, ad, REPLAY_SIZE)

    obs, _ = env.reset(seed=seed)
    curve = []
    use_confidence = (mode == "v4") and hasattr(env, 'last_gate_score')
    ...
    for step in range(1, TRAIN_STEPS+1):
        ...
        conf = env.last_gate_score if use_confidence else 1.0
        buf.add(obs, a, r, obs2, float(d and not tr), confidence=conf)
        obs = obs2
        ...
        if step % EVAL_INTERVAL == 0:
            ret = evaluate(agent, env_cls)
            curve.append((step, ret))
            gate_info = ""
            if hasattr(env, 'gate_stats'):
                gs = env.gate_stats()
                if gs: gate_info = f"  gate_median={gs['median']:.3f} frac_zero={gs['frac_zero']:.2f}"
            log_fn(f"  step {step:>6d} | return={ret:7.1f}  α={agent.alpha:.4f}{gate_info}")
...
def main():
    ...
    parser.add_argument("--mode", default="v3", choices=["v2", "v3", "v4"])
    args = parser.parse_args()
    mode = args.mode

    log, log_file = make_logger(mode)
    log(f"[{mode}] Device: {DEVICE}")

    env_cls = WindHopperEnv

    log(f"\n[A] Paired data + SINDy")
    SA, delta_s, real_transitions = collect_paired(env_cls, N_COLLECT, log)
    ...
    # v1 Raw Sim baseline
    RAW_SIM_BASELINE = [(40000, 380.0), (45000, 384.4), (50000, 555.2)]
    log("  Raw Sim baseline (v1): last 3 = [380.0, 384.4, 555.2], avg=439.9")
    ...
    curve = train(make_gated, env_cls, f"Gated+Aug ({mode})", log, mode,
                  real_data=real_transitions)

    # Results
    fr = np.mean([r for _, r in RAW_SIM_BASELINE[-3:]])
    if curve:
        fc = np.mean([r for _, r in curve[-3:]])
        gap = (fc - fr) / max(abs(fr), 1) * 100
        log(f"\n{'='*50}")
        log(f"Raw Sim          (v1 baseline): {fr:.1f}")
        log(f"{mode} Gated+Aug  (last 3 avg): {fc:.1f}")
        log(f"Gap: {gap:+.1f}%")
        if gap > 10:   log(">>> SOLID — paper viable")
        elif gap > 5:  log(">>> MARGINAL — need more envs")
        else:          log(">>> WEAK — check gate distribution")
        log(f"{'='*50}")

    # Plot
    fig, ax = plt.subplots(figsize=(10, 5))
    sx, sy = zip(*RAW_SIM_BASELINE)
    ax.plot(sx, sy, label="Raw Sim (v1)", color="steelblue", lw=2, ls="--")
    if curve:
        cx, cy = zip(*curve)
        ax.plot(cx, cy, label=f"{mode} Gated+Aug", color="darkorange", lw=2)
    ax.set_xlabel("Steps"); ax.set_ylabel("Real env return")
    ax.set_title(f"WindHopper: {mode} (RE-SAC)")
    ax.legend(); ax.grid(alpha=0.3); plt.tight_layout()
    out = os.path.join(OUT_DIR, f"mvp_fast_{mode}.png")
